...
pitch          	= 3   ; // distance between the teath
beltWidth       	= 10   ; // the width/height of the belt. The (vertical) size of the pulley is adapted to this.
beltThickness   	= 1.04; // thickness of the part excluding the notch depth!
notchDepth      	= 1.5 ; // make slightly bigger than actual, to avoid outward curvature in the inner solid part of the pulley
toothWidth     	= 1.5 ; // Teeth of the PULLEY, that is.
...
timingPulley
	...
	,9.61  	//shaftDiameter   	// the shaft at the center, will be subtracted from the pulley. Better be too small than too wide.
	...
	,6.35  	//hubSetScrewDiameter// use either a set screw or nut on a shaft. Set to 0 to not use a set screw.
...
timingPulley
	...
	,2		//flanges          	// the rims that keep the belt from going anywhere, 0, 1 (on the top) or 2
	,0.5	//flangeHeight    	// how much the flanges angle in toward the inner diameter of the pulley.
	,6.35 	//shaftDiameter   	// the shaft at the center, will be subtracted from the pulley. Better be too small than too wide.
	,0  	//shaftFlatDiameter	// the diameter of the shaft when measured at a flat milled in the shaft. 0 if the shaft is round.
